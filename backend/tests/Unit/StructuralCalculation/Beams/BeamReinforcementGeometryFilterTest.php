...
function geometryFilterResult(BeamReinforcementCandidatesResult $candidates, float $width = 300): BeamReinforcementGeometryCandidatesResult
{
    return beamReinforcementGeometryFilter()->filter(
        $candidates, new BeamGeometry(6500, $width, 600), geometryCover(), BeamFlexuralDetailingAssumptions::mvp(),
...
it('uses the EC2-05 nominal cover and existing transverse-detailing assumption', function () {
    ...
    $cover = app(NominalCoverCalculator::class)->calculate(
        new CoverCalculationInput(CoverMode::AUTO, [ExposureClassCode::XC4], ConcreteStrengthClass::C30_37, 50, BeamFlexuralDetailingAssumptions::mvp()->transverseReinforcementDiameter),
        $profile,
    );
    $result = beamReinforcementGeometryFilter()->filter(
        ...
        BeamFlexuralDetailingAssumptions::mvp(), new BeamReinforcementDetailingAssumptions, $profile->reinforcementSpacingRequirements,
    );
