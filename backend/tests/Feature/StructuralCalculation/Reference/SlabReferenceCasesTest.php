...
it('matches independently established full-pipeline slab references', function (array $input, array $expected) {
    ...
    qaSlabAssertClose($details->characteristicActions->selfWeight, $expected['selfWeight'], 1e-10);
    expect(array_keys(get_object_vars($details)))->toBe(['overallStatus', 'ulsStatus', 'slsStatus', 'governingVerification', 'assumptions', 'characteristicActions', 'combinations', 'internalForces', 'flexure', 'mainReinforcement', 'secondaryReinforcement', 'serviceability', 'warnings']);
})->with([
